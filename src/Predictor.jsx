            ...
            <div style={{padding:"1.25rem 1rem 1rem",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"0.65rem",flexWrap:"wrap",gap:"0.35rem"}}>
                {winner&&(()=>{
                  ...
                  const isMedium = pct >= 55 && pct < 65;
                  const isWeak = pct < 55;
                  const icon = isStrong?"🔥":isMedium?"⚡":"⚖️";
                  const label = isStrong?"STERKE FAVORIET":isMedium?"LICHTE FAVORIET":"EVENWICHTIG";
                  const color = winner===home?"#4ade80":winner===away?"#f87171":"#fbbf24";
                  const bg = winner===home?"rgba(34,197,94,0.1)":winner===away?"rgba(239,68,68,0.08)":"rgba(234,179,8,0.08)";
                  const border = winner===home?"rgba(34,197,94,0.2)":winner===away?"rgba(239,68,68,0.15)":"rgba(234,179,8,0.15)";
                  return(
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0.25rem"}}>
                      <span style={{display:"inline-flex",alignItems:"center",gap:"0.35rem",padding:"0.2rem 0.65rem",background:bg,border:`1px solid ${border}`,borderRadius:"20px",fontSize:"0.65rem",fontWeight:600,color}}>
                        {winner==="Gelijkspel"?"🤝 Gelijkspel waarschijnlijkst":`${f(winner)} ${winner} wint — ${pct.toFixed(0)}%`}
                      </span>
                      <span style={{display:"inline-flex",alignItems:"center",gap:"0.3rem",padding:"0.15rem 0.6rem",background:isStrong?"rgba(255,100,0,0.1)":isWeak?"rgba(255,255,255,0.04)":"rgba(255,200,0,0.08)",border:`1px solid ${isStrong?"rgba(255,100,0,0.2)":isWeak?"rgba(255,255,255,0.08)":"rgba(255,200,0,0.15)"}`,borderRadius:"20px",fontSize:"0.6rem",fontWeight:700,color:isStrong?"#ff6400":isWeak?"rgba(255,255,255,0.3)":"#fbbf24"}}>
                        {icon} {label}
                      </span>
                    </div>
                  );
                ...
                <div style={{background:"rgba(168,85,247,0.05)",border:"1px solid rgba(168,85,247,0.12)",borderRadius:"12px",padding:"0.75rem"}}>
                  ...
                  {oddsResult&&allOddsValid&&(
                    <div style={{display:"flex",flexDirection:"column",gap:"0.3rem",marginBottom:"0.5rem"}}>
                      ...
                      {(()=>{
                        const edges = [
                          {e:edgeH,l:`${shortName(home)} wint`,o:parseFloat(oH)},
                          {e:edgeD,l:"Gelijkspel",o:parseFloat(oD)},
                          {e:edgeA,l:`${shortName(away)} wint`,o:parseFloat(oA)},
                        ].filter(x=>x.e!==null);
                        const bestBet = edges.reduce((best,x)=>x.e>best.e?x:best,{e:-999});
                        const noValue = edges.every(x=>Math.abs(x.e)<3);
                        return(<>
                          {noValue?(
                            ...
                            <div style={{padding:"0.5rem 0.7rem",background:"rgba(0,232,122,0.06)",border:"1px solid rgba(0,232,122,0.15)",borderRadius:"10px",marginTop:"0.3rem"}}>
                              <div style={{fontSize:"0.62rem",fontWeight:700,color:"#00e87a",marginBottom:"0.3rem"}}>🎯 BESTE BET</div>
                              <div style={{fontSize:"0.75rem",fontWeight:700,color:"#fff"}}>{bestBet.l} @ {bestBet.o.toFixed(2)}</div>
                              <div style={{fontSize:"0.62rem",color:"rgba(255,255,255,0.5)",marginTop:"0.2rem"}}>
                                Edge: +{bestBet.e.toFixed(1)}% · {bestBet.e>=8?"Sterke value 🔥":bestBet.e>=5?"Goede value ⚡":"Kleine value 📈"}
                              </div>
                              <div style={{fontSize:"0.58rem",color:"rgba(255,255,255,0.3)",marginTop:"0.3rem"}}>
                                💡 Bankroll advies: zet max {bestBet.e>=8?"3-4%":bestBet.e>=5?"2-3%":"1-2%"} in
                              </div>
                            </div>
                          )}
